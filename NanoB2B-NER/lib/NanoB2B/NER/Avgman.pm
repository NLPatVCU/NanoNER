...
sub avg_file{
	...
	#get the averaged weka data, individual weka data, and majority sense baseline
	...
	my $WEKA_SAVE;
	open ($WEKA_SAVE, ">", "$file") || die ("Weka file $file could not be made in the _WEKAS directory! \n$!");
	...
	foreach my $key (sort keys %avg){
		...
		for(my $a = 0; $a < $arrLen; $a++){
			...
			my $entry = $arr[$a];
			#try to align the values based on the length
			my $tab = "";
...
sub averageWekaData{
	...
	foreach my $item (@sets){
		...
		foreach my $bucket(@allBuckets){
			...
			open (WEKA, "$file") || die ("Unable to import $file from _WEKAS directory\n $!");
			...
			#file doesn't have any contents for some reason
			if($len == 0){
				$uniSub->printColorDebug("red", "**ERROR: NOTHING FOUND FOR $file!**\n");
				my $n = @wekaAttr;
				my @values = (0) x $n;
		...
		foreach my $bucket (@allBuckets){
			...
			for(my $e = 0; $e < $wekaLen; $e++){
				my $entry = $wekaSet[$e];
				if($entry ne "NaN" and $entry ne "?"){		#if value is unknown (from WEKA)
					$averages[$e] += $entry;
				}
			}
		}
		...
		#add the averages for the specific feature
...
sub individualWekaData{
	...
	foreach my $item (@sets){		#for each feature specified
		my @data = ();

		foreach my $bucket(@allBuckets){	#for each bucket in the set
			...
			#if the contents of the file are empty for some reason
			if($len == 0){
				$uniSub->printColorDebug("red", "**ERROR: NOTHING FOUND FOR $file!**\n");
				my $n = @wekaAttr;
				my @values = (0) x $n;
...
sub countInst{
	...
	foreach my $line(@set){
		...
		foreach my $word(@words){
			#print($word . "\n");
			#if the word doesn't have a entity tag
			if(!($word =~ /([\s\S]*($entId)$)/)){
				$no_inst++;		#count the words not annotated
			}

			$all_inst++;		#count all the words
		}
	}
...
sub retag{
	...
	my $charact = 0;
	foreach my $word (@words){
		if($charact){				#if currently annotating
			if($word eq "<end>"){		#stop tagging annotations
				$charact = 0;
			}else{						#add the entity tagger to the words
				my $charWord = "$word"."$entId"; 
				push @newSet, $charWord;
			}
		}else{
			if($word eq "<start:$id>"){		#start annotating
				$charact = 1;
			}else{							#add normal word
				push @newSet, $word;
			}
		}
	}
